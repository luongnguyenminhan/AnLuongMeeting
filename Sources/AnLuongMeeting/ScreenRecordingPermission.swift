...
enum ScreenRecordingPermission {
    ...
    static func checkAtLaunch() {
        ...
        AnLuong Meeting uses the macOS Screen Recording permission to record system audio. No video is ever saved.

        • New install: click "Open Settings" and enable AnLuong Meeting in the list.
        • Upgraded from an older version: the old grant is stale — toggle AnLuong Meeting OFF and back ON.

        AnLuong Meeting will offer to relaunch once access is granted.
        """
        alert.addButton(withTitle: "Open Settings")
        alert.addButton(withTitle: "Later")
        guard alert.runModal() == .alertFirstButtonReturn else { return }

        // On fresh installs this fires the system prompt and adds AnLuong Meeting
    ...
    private static func offerRelaunch() {
        ...
        alert.informativeText = "macOS applies the permission on the next launch. Relaunch AnLuong Meeting now?"
