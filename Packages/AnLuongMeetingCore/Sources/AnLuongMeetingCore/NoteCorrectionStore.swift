...
public struct NoteCorrectionStore: Sendable {
    private let fileURL: URL
    private let fileManager: FileManager

    public init(directory: URL, baseName: String, fileManager: FileManager = .default) {
        self.fileURL = directory.appendingPathComponent(baseName + ".note-corrections.json")
        self.fileManager = fileManager
    }

    public func load() -> [NoteCorrection] {
    ...
    public func save(_ corrections: [NoteCorrection]) throws {
...
public func wrapCorrectionsAsLinks(in text: String, corrections: [NoteCorrection]) -> String {
    var result = text
