...
struct GlossaryView: View {
    ...
    init(pending: IOSPendingWorkCoordinator) {
        self.pending = pending
        _memory = State(initialValue: pending.memoryStore.load())
    }
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text("AnLuongMeeting learns names, project/jargon terms, and note-style preferences from your meetings, then reuses them to correct future transcripts and notes.")
            }
        ...
        .onChange(of: pending.isBackfillingMemory) { _, isRunning in
            if !isRunning { memory = pending.memoryStore.load() }
        }
    }
    ...
    private var backfillRow: some View {
        VStack(alignment: .leading, spacing: 6) {
            Button {
                pending.backfillMemoryFromExistingMeetings()
            } label: {
                Label("Generate from existing meetings", systemImage: "sparkles")
            }
            .disabled(pending.isBackfillingMemory)

            if pending.isBackfillingMemory {
                ProgressView(value: Double(pending.backfillProgress.current), total: Double(max(pending.backfillProgress.total, 1)))
                Text("Analyzing \(pending.backfillProgress.current) of \(pending.backfillProgress.total) meetings…")
                    .font(.caption)
